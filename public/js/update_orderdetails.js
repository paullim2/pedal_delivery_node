...
updateOrderDetailsForm.addEventListener("submit", function (e) {
    ...
    let inputFoodID = document.getElementById("input-food_id-update");

    // Get the values from the form fields
    let orderDetailsValue = inputOrderDetailsID.value;
    let foodIDValue = inputFoodID.value;
    
    // currently the database table for bsg_people does not allow updating values to NULL
    // so we must abort if being bassed NULL for homeworld

    if (isNaN(orderDetailsValue)) 
    {
        return;
    }

    if (isNaN(foodIDValue)) 
    {
        return;
    }
    ...
    let data = {
        orderdetails_id: orderDetailsValue,
        food_id: foodIDValue
    }
